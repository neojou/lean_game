...
我留了隱藏提示；若需要，從頭再試一次，並點「顯示更多提示！」。
    ...
    theorems = listOf(
        InventoryItem(InventoryKind.Theorem, "zero_le", "zero_le ", "`zero_le x`：`0 ≤ x`。", theoremTab = TheoremTab.Le),
        InventoryItem(InventoryKind.Theorem, "add_zero", "add_zero", "`a + 0 = a`。", theoremTab = TheoremTab.Plus),
        InventoryItem(InventoryKind.Theorem, "add_succ", "add_succ", "`a + succ b = succ (a + b)`。", theoremTab = TheoremTab.Plus),
        InventoryItem(InventoryKind.Theorem, "succ_add", "succ_add", "`succ a + b = succ (a + b)`。", theoremTab = TheoremTab.Plus),
        InventoryItem(InventoryKind.Theorem, "add_assoc", "add_assoc", "`(a + b) + c = a + (b + c)`。", theoremTab = TheoremTab.Plus),
        InventoryItem(InventoryKind.Theorem, "succ_eq_add_one", "succ_eq_add_one", "`succ n = n + 1`。", theoremTab = TheoremTab.Plus),
        InventoryItem(
            InventoryKind.Theorem, "le_trans", "le_trans ",
            "`le_trans x y z`：若 `x ≤ y` 且 `y ≤ z`，則 `x ≤ z`。",
            theoremTab = TheoremTab.Le,
        ),
        InventoryItem(
    ...
    definitions = listOf(
        InventoryItem(InventoryKind.Definition, "ℕ", "ℕ", "本遊戲的自然數 `MyNat`：`0` 與 `succ`。", docId = "nat"),
        InventoryItem(InventoryKind.Definition, "≤", "≤", "`a ≤ b` 定義為「存在 `c` 使得 `b = a + c`」。", docId = "le"),
        InventoryItem(InventoryKind.Definition, "∨", "∨", "邏輯或。證它用 `left` / `right`；用它用 `cases`。", docId = "or"),
    ),
